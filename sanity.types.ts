...
// Query: *[_type == "blog-slug" && slug.current == $slug][0]{  title,  slug,  _createdAt,  _updatedAt,}
...
// Query: *[_type == "course-slug" && slug.current == $slug][0]{    title,    slug,    _createdAt,    _updatedAt,  }
...
// Query: *[_type == "service-slug" && slug.current == $slug][0]{    title,    slug,    _createdAt,    _updatedAt,    featuredImage{          crop,  hotspot,  "darkScore": (    (coalesce(asset->metadata.palette.darkVibrant.population, 0)     + coalesce(asset->metadata.palette.darkMuted.population, 0))    /    (      coalesce(asset->metadata.palette.darkVibrant.population, 0)      + coalesce(asset->metadata.palette.darkMuted.population, 0)      + coalesce(asset->metadata.palette.dominant.population, 0)      + coalesce(asset->metadata.palette.lightVibrant.population, 0)      + coalesce(asset->metadata.palette.lightMuted.population, 0)      + coalesce(asset->metadata.palette.muted.population, 0)    )  ),  asset->{    _id,    title,    altText,    description,    metadata {      lqip,      dimensions {        aspectRatio,        width,        height      },      palette {        dominant {          background,          foreground,          population        }      }    }  }    },  }
